...
export const SettingsView: React.FC<SettingsViewProps> = ({ settings, onUpdateSettings, labels, onUpdateLabels, onClearData, onRepairWorkspace }) => {
  ...
  const sidebarItems = [
    { id: 'ai', title: 'Models & Keys', subtitle: 'Connect to AI providers', icon: Zap },
    { id: 'appearance', title: 'Look & Feel', subtitle: 'Colors and themes', icon: Palette },
    { id: 'input', title: 'Typing', subtitle: 'Shortcuts and behavior', icon: Keyboard },
    { id: 'workspace', title: 'General', subtitle: 'Naming and reset options', icon: Briefcase },
    { id: 'permissions', title: 'Instructions', subtitle: 'How the AI should behave', icon: Shield },
    { id: 'labels', title: 'Tags', subtitle: 'Organize your chats', icon: Tag },
    { id: 'shortcuts', title: 'Hotkeys', subtitle: 'Keyboard navigation', icon: Keyboard },
  ];
          ...
          <div className="max-w-2xl mx-auto py-16 px-8">
              ...
              {activeTab === 'ai' && (
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-400">
                      <div className="bg-[var(--bg-secondary)]/50 border border-[var(--border)] rounded-3xl p-8 space-y-8">
                          <h3 className="font-bold text-lg tracking-tight">API Keys</h3>
                      ...
                      <div className="space-y-4">
                          <h3 className="font-bold text-lg mb-1 tracking-tight px-2">Enabled Models</h3>
                          <div className="grid grid-cols-1 gap-3">
                            ...
                            <ModelList title="Others" models={[...DEEPSEEK_MODELS, ...MOONSHOT_MODELS]} visibleModels={settings.visibleModels} onToggle={toggleModel} />
                          </div>
                      </div>
                  </div>
              )}
              ...
              {activeTab === 'appearance' && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-400">
                      <div className="bg-[var(--bg-secondary)]/50 border border-[var(--border)] rounded-3xl p-8">
                          <h3 className="font-bold text-lg mb-6 tracking-tight">Theme</h3>
                          <div className="grid grid-cols-2 gap-4">
                              <div onClick={() => onUpdateSettings({ ...settings, theme: 'light' })} className={`p-6 rounded-2xl border transition-all cursor-pointer flex flex-col items-center gap-4 ${settings.theme === 'light' ? 'border-[var(--text-main)] bg-[var(--bg-elevated)] text-[var(--text-main)]' : 'border-[var(--border)] text-[var(--text-muted)] hover:bg-[var(--bg-elevated)]/50'}`}>
                                  <div className="w-12 h-12 rounded-full bg-white border border-gray-200" /> <span className="text-[13px] font-bold">Light</span>
                              </div>
                              <div onClick={() => onUpdateSettings({ ...settings, theme: 'dark' })} className={`p-6 rounded-2xl border transition-all cursor-pointer flex flex-col items-center gap-4 ${settings.theme === 'dark' ? 'border-[var(--text-main)] bg-[var(--bg-elevated)] text-[var(--text-main)]' : 'border-[var(--border)] text-[var(--text-muted)] hover:bg-[var(--bg-elevated)]/50'}`}>
                                  <div className="w-12 h-12 rounded-full bg-neutral-900 border border-neutral-800" /><span className="text-[13px] font-bold">Dark</span>
                              </div>
                          </div>
                      </div>
                      <div className="bg-[var(--bg-secondary)]/50 border border-[var(--border)] rounded-3xl p-8">
                          <h3 className="font-bold text-lg mb-6 tracking-tight">Color Accent</h3>
              ...
              {activeTab === 'input' && (
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-400">
                      <div className="bg-[var(--bg-secondary)]/50 border border-[var(--border)] rounded-3xl p-8 space-y-8">
                          <div><h3 className="font-bold text-lg mb-1 tracking-tight">How to send</h3><p className="text-[13px] text-[var(--text-muted)]">Choose your preferred keyboard shortcut.</p></div>
                          <div className="grid grid-cols-2 gap-4">
                              <div onClick={() => onUpdateSettings({...settings, sendKey: 'Enter'})} className={`p-5 rounded-2xl border cursor-pointer flex items-center justify-between transition-all ${settings.sendKey === 'Enter' ? 'border-[var(--text-main)] bg-[var(--text-main)]/5 text-[var(--text-main)]' : 'border-[var(--border)] text-[var(--text-dim)] hover:border-[var(--text-muted)]'}`}><span className="text-[13px] font-bold">Enter</span><Keyboard className="w-4 h-4 opacity-50" /></div>
                              <div onClick={() => onUpdateSettings({...settings, sendKey: 'Ctrl+Enter'})} className={`p-5 rounded-2xl border cursor-pointer flex items-center justify-between transition-all ${settings.sendKey === 'Ctrl+Enter' ? 'border-[var(--text-main)] bg-[var(--text-main)]/5 text-[var(--text-main)]' : 'border-[var(--border)] text-[var(--text-dim)] hover:border-[var(--text-muted)]'}`}><span className="text-[13px] font-bold">Ctrl + Enter</span><Keyboard className="w-4 h-4 opacity-50" /></div>
                          </div>
                      </div>
                  </div>
              )}
                  ...
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-400 space-y-6">
                      <div className="bg-[var(--bg-secondary)]/50 border border-[var(--border)] rounded-3xl p-8 space-y-8">
                        <div className="flex items-start justify-between">
                            <div><h3 className="font-bold text-lg mb-1 tracking-tight">Details</h3><p className="text-[13px] text-[var(--text-muted)]">Customize your space.</p></div>
                      ...
                      <div className="pt-4 flex flex-col gap-3">
                          <button onClick={onRepairWorkspace} className="w-full px-6 py-4 bg-[var(--bg-elevated)] border border-[var(--border)] rounded-2xl text-[13px] font-bold flex items-center justify-center gap-2 hover:bg-[var(--border)] transition-colors"><Wrench className="w-4 h-4" />Repair Workspace</button>
                          <button onClick={onClearData} className="w-full px-6 py-4 bg-red-950/20 text-red-400 border border-red-900/20 rounded-2xl text-[13px] font-bold flex items-center justify-center gap-2 hover:bg-red-900/30 transition-colors"><AlertTriangle className="w-4 h-4" />Delete Everything</button>
                      </div>
                  </div>
              )}
              ...
              {activeTab === 'permissions' && (
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-400">
                       <div className="bg-[var(--bg-secondary)]/50 border border-[var(--border)] rounded-3xl p-8 space-y-6">
                          <div><h3 className="font-bold text-lg mb-1 tracking-tight">AI Behavior</h3><p className="text-[13px] text-[var(--text-muted)]">Global rules that the AI will always follow.</p></div>
                          <textarea value={settings.baseKnowledge} onChange={(e) => onUpdateSettings({...settings, baseKnowledge: e.target.value})} placeholder="e.g. Always keep answers short..." className="w-full h-48 bg-[var(--bg-elevated)]/50 border border-[var(--border)] rounded-2xl px-5 py-5 text-[14px] focus:outline-none focus:border-[var(--text-muted)] text-[var(--text-main)] resize-none" />
                      </div>
                  </div>
              )}
                  ...
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-400">
                       <div className="bg-[var(--bg-secondary)]/50 border border-[var(--border)] rounded-3xl p-8 space-y-6">
                          <div className="grid grid-cols-1 gap-3">
                              ...
                              <ShortcutItem keys={['Alt', 'A']} label="Switch Mode" />
                              <ShortcutItem keys={['Alt', 'T']} label="Toggle Tasks" />
                  ...
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-400">
                      <div className="bg-[var(--bg-secondary)]/50 border border-[var(--border)] rounded-3xl p-8">
                          <div className="flex items-center justify-between mb-10">
                              <h3 className="font-bold text-lg mb-1 tracking-tight">Tags</h3>
                              <button onClick={() => setIsAddingLabel(true)} className="bg-white text-black px-4 py-2 rounded-xl text-xs font-bold hover:opacity-90">Add Tag</button>
                          </div>
                          {isAddingLabel && (
                              <div className="flex items-center gap-3 mb-8 p-5 rounded-2xl bg-[var(--bg-elevated)] border border-[var(--text-muted)]/20">
                          ...
                          <div className="space-y-1.5">
                              {labels.map(label => (
                                  <div key={label.id} className="flex items-center justify-between p-4 hover:bg-[var(--bg-elevated)]/40 rounded-2xl group transition-all">
                                      ...
                                      <button onClick={() => onUpdateLabels(labels.filter(l => l.id !== label.id))} className="text-[var(--text-dim)] hover:text-red-400 opacity-0 group-hover:opacity-100 p-2"><Trash2 className="w-4 h-4" /></button>
                                  </div>
                              ))}
                          </div>
                      </div>
